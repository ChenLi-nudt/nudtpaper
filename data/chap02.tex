...
三维堆叠集成技术~\upcite{xie-jetc2006, bower2008handbook}也可以分为两类：一类是\texttt{单晶堆叠方法（Monolithic Approach）}，该方法在单一晶元上完成三维集成电路所有的设计制作流程，
...
三维堆叠集成技术能够有效克服全局连线延迟的瓶颈，为集成电路性能持续发展提供了解决方案~\upcite{joyner2001stochastic}。连线长度的缩减主要带来两方面巨大优势：延迟和功耗降低。
延迟的降低是由于平均线长和关键路径的缩短，而总线长的缩短也自然使得功耗显著降低~\upcite{joyner2001stochastic,vaidyanathan2007architecting,ouyang2009arithmetic,egawa2009evaluation}。
...
解决互连瓶颈、消除``存储墙''问题~\upcite{jacob2009mitigating,loh-stack,loh-ieeemicro2007}。通过将存储器堆叠在处理器芯片上，利用TSV带来的通信优势提供远高于二维结构的通信带宽。
...
因此三维堆叠集成技术潜在地提供了一种相比于传统集成电路更低成本的解决方案~\upcite{dong2009cost}。


%三维堆叠的挑战
虽然三维堆叠集成技术带来了巨大的性能优势和在系统结构设计的机会，但在其广泛应用到未来的计算机系统前仍然需要解决几个重要的挑战：

\textbf{热问题}。功耗和热问题在传统的二维集成电路设计中已经成为一个重要的问题~\upcite{sapatnekar2009addressing}。
...
\begin{figure}[htbp] % use float package if you want it here
  \centering
  \includegraphics[width=\textwidth]{/Figs_background/25Dstacking}
  \caption{2.5维堆叠集成电路}
...
对于当前的三维堆叠存储标准JEDEC HBM~\upcite{hbm2}，提供1024位数据信号在1Gbps的频率工作，每个堆叠存储能够提供128GB/s。 
...
\label{sec:gpu}
%GPU历史
计算机辅助绘图最早出现在二十世纪六十年代，如Ivan Sutherland提出的Sketchpad~\upcite{sutherland1964sketchpad}
...
从2001年开始，NVIDIA以顶点着色器~\upcite{lindholm2001user}和像素着色器的形式将可编程性引入了GPU，出现了NVIDIA Geforce 3图形处理器。
...
在NVIDIA Fermi架构中~\upcite{fermi}，GPU出现了缓存以暂存读或写的数据到片内。
随后的改进包括AMD的Fusion架构~\upcite{amd-fusion}，它在同一芯片上集成了CPU和GPU，以及动态并行性，可以从GPU本身启动线程。
最近，NVIDIA的Volta~\upcite{volta}推出了Tensor Cores等功能，专门用于机器学习加速。
...
虽然正在不断努力开发直接在GPU上提供I/O服务的应用程序编程接口（API），但到目前为止这些CPU已经具有I/O的能力~\upcite{silberstein2016gpunet}\upcite{Silberstein:20139}。
这些API的功能在CPU和GPU之间提供了简单的通信接口，隐藏了复杂的管理过程。


\begin{figure}[htb]
\centering
\subfloat[分立式CPU和GPU]{\includegraphics[width=.6\textwidth]{/Figs_background/discrete}} \qquad
\subfloat[集成式CPU和GPU]{\includegraphics[width=.6\textwidth]{/Figs_background/integrated}} 
\caption{CPU和GPU典型结构的抽象图}
\label{fig:CPUGPU}
\end{figure}

如图~\ref{fig:CPUGPU}所示的是包含CPU和GPU的典型系统的抽象图。图~\ref{fig:CPUGPU}(a)是典型的分立式GPU配置，包括连接CPU和GPU的总线（例如，PCIe或NVLink），广泛用于NVIDIA的GPU体系结构，如NVIDIA的Volta GPU。
...
这可以利用GPU虚拟内存~\upcite{gelado2010asymmetric}实现。NVIDIA称之为``统一内存''（Unified Memory）。
对于CPU和GPU都集成到同一芯片上并共享相同的内存的情况，程序员不需要控制CPU内存到GPU内存的拷贝。
但是，由于CPU和GPU使用缓存，而其中一些缓存可能是私有的，因此可能存在Cache一致性问题~\upcite{power2013heterogeneous}，需要开发人员来解决。
...
然而，Seo等人~\upcite{seo2011performance}观察到一个现象，在OpenCL~\upcite{kaeli2015heterogeneous}上针对一个体系结构（例如，GPU）仔细优化过的代码可能在另一个体系结构（例如，CPU）上执行效果非常不好~\upcite{seo2011performance}。

图~\ref{fig:csaxpy}提供了用于CPU实现一个大家熟知的操作，即单精度标量值A乘以矢量值X加矢量值Y的的C代码，称为SAXPY。 
SAXPY是众所周知的基本线性代数软件库（BLAS）~\upcite{Lawson:1979:BLA:355841.355847}的一部分，可用于实现更高级别的矩阵运算，如高斯消元法。 
鉴于其简单性和实用性，它经常被用作教授计算机体系结构~\upcite{hp-aqa}的示例。
...
在示例中，CPU在第17行调用CUDA的内核函数配置语法的启动GPU上的计算。
...
在示例中，未指定y和z维度，因此所有线程的y和z线程块和线程坐标的值都为零。
...
在示例中，\texttt{blockDim.x}在第17行被置为256。
...
\textbf{异构系统中的并发管理}。 
Kayiran等人~\upcite{cpugpu-micro}提出了一种并发限制方案来限制GPU多线程的并行度，减少支持多任务的CPU-GPU异构系统中的内存和网络竞争。
...
为了解决CPU为中心的技术限制warp并行度对GPU性能的影响，作者提出了一种以最大化整体系统吞吐量为目标的更均衡的技术。
...
\textbf{异构系统一致性}。
Power等人~\upcite{power2013heterogeneous}提出了一种硬件机制，以有效地支持CPU和GPU集成系统上的高速缓存一致性。
...
Lee和Kim~\upcite{lee2012tap}评估了在异构环境中管理CPU核心和GPU核心之间的共享最后一级缓存（LLC）的效果。
...
Kayiran等人~\upcite{kayiran2013neither}提出限制分配给每个核心的线程块的数量，以减少由线程超额配置引起的内存系统中的竞争。 
...
Gebhart等人~\upcite{gebhart}介绍了使用两级warp调度策略来提高能效。
...
Narasiman等人~\upcite{largewarps}提出的两级调度程序侧重于通过允许线程组在不同时间达到相同的长延迟操作来提高性能。
这有助于维护一段时间内的缓存和行缓冲区的局部性。
然后，系统可以通过在获取组之间切换来隐藏长延迟操作。

针对硬件线程调度对GPU中缓存管理的影响。Rogers等人~\upcite{rogers2012cache}提出了提出了缓存意识warp调度策略（Cache-Conscious Wavefront Scheduling，CCWS）。
这是一种自适应硬件机制，它利用一种新颖的warp内部局部性特征检测来捕获由于过度竞争缓存容量而损失的局部性优势。
不同于那些改进缓存替换策略的方法，CCWS优化访存模式以避免共享L1缓存的抖动。
实验证明CCWS优于缓存替换策略的优化。

Jog等人~\upcite{osp-isca13}在GPU上探索预取感知的warp调度策略。
...
Jog等人~\upcite{owl-asplos13}还提出了一种线程块感知的warp调度策略。
...
Park等人~\upcite{park2015chimera}针对GPU上抢占式多任务处理的挑战，采用了更为宽松的幂等性定义，直接丢弃线程块的计算。更宽松的幂等性定义涉及检测执行是否从线程开始执行至今是幂等的。
...
% 在传统的上下文切换方法的基础上，Tanasic等人提出了排空执行方法进行抢占~\upcite{tanasic2014enabling}。
% 这种方法对于相对较短的内核函数非常有效。
% Park等人提出了丢弃执行的方法~\upcite{park2015chimera}。
% 这种方法对于满足幂等性要求的抢占内核函数可以实现零抢占延迟。
% 此外，他们的工作还包含将上下文切换、排空执行和丢弃执行基于线程块的执行时间混合选择执行。
Wang等人设计了细粒度动态共享机制，SMK~\upcite{wang2016simultaneous}。
他们的设计使得基于线程块的细粒度上下文切换能够实现较低的抢占延迟。

在缩减上下文大小加速多任务切换方面，iGPU~\upcite{menon2012igpu}的观点是，可以在满足幂等性代码区域之间的边界处备份和恢复上下文。 
他们利用活跃度分析来确定恢复点。
这些点的活跃度寄存器数量相对较少。
Lin等人提出三种技术实现轻量级上下文切换，包括本地上下文存储、活跃度分析和上下文压缩等~\upcite{lin2016enabling}。


\textbf{异构系统的软件优化。}
Kim和Batten~\upcite{kim2014accelerating}提出在GPU中为每个SIMT核心添加细粒度硬件工作清单。
...
他们评估了这些硬件机制运行在lonestar GPU基准测试集~\upcite{lonestar}的不规则应用程序的各种实现，包括利用拓扑的方法和数据驱动的方法。
...
Wang和Yalamanchili等人~\upcite{dynparallel-iiswc14}研究了在NVIDIA Kepler GPU上使用CUDA Dynamic Parallelism的开销，并发现这些开销可能很大。
...
Wang等人~\upcite{wang2016dynamic}之后提出了动态线程块启动策略，他们修改了CUDA编程模型使得从设备启动的内核函数可以共享硬件队列的资源。
这可以获取更高的并行度和更好的资源利用率。
他们的策略的关键是可以动态的组合一起启动的内核函数。
这是通过管理一个线程块链表来维护，并需要修改硬件。
实验评估通过修改GPGPU-sim~\upcite{gpgpu-sim}完成，实验表明这个方法相比于基准方法提升了1.4倍的性能，相比于高度优化的CUDA方案也提升了1.2倍的性能。
Wang等人~\upcite{wang2016laperm}还探索了线程块在不同核心被动态启动的影响。
...
% 等引用未加详细测试）。看看这个例子，关于书的\upupcite{tex, companion}，
% 还有这些\upupcite{Krasnogor2004e, clzs, zjsw}，关于杂志的\upupcite{ELIDRISSI94,
%   MELLINGER96, SHELL02}，硕士论文\upupcite{zhubajie, metamori2004}，博士论文
% \upupcite{shaheshang, FistSystem01}，标准文件\upupcite{IEEE-1363}，会议论文\upupcite{DPMG,kocher99}，%
% 技术报告\upupcite{NPB2}。中文参考文献\upupcite{cnarticle}\textsf{特别注意}，需要在\verb|bibitem|中
...
% 有时候不想要上标，那么可以这样 \upcite{shaheshang}，这个非常重要。
